...
#xlim([-1, 2]);
#ylim([0, 1.5 * maximum(real([a, b]))]);
#ylabel("tdm strength")

ax2 = bar([0, 1], [real(a), real(b)], ylims=(0, maximum([real(a), real(b)]) + 0.5), legend=false)
title!("TDM strengths")
...
#xlabel("time");
#ylabel("Corr. func. ⟨μ(t) μ₀₁ ρ₀⟩");
# spectrum
#ax5 = subplot(236)
#plot(-[E₁, E₁], [0, 0.5])
#plot(-[E₂, E₂], [0, 0.5])
#plot(ω, spec, "k")
#plot(-[energies[2], energies[2]], [0, 0.5], "k--")
#plot(-[energies[3], energies[3]], [0, 0.5], "k--")
...
a_ops = [sqrt(0.32) * one(b_mon) ⊗ (j12 * j21), debye, sqrt(0.32) * (j12 * j21) ⊗ one(b_mon), debye]
...
# and plot
plot!(ax4, tout, [real(es1), real(es2)], xlims=(0, 200))
...
f = plot(ax1, ax2, ax3, ax4, ax5, layout=l, size=(1000, 500))
display(f)
